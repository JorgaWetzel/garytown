...
powershell Invoke-Expression -Command (Invoke-RestMethod -Uri https://raw.githubusercontent.com/gwblok/garytown/master/Dev/CloudScripts/Production.ps1)
...
.EXAMPLE
    powershell iex (irm https://raw.githubusercontent.com/gwblok/garytown/master/Dev/CloudScripts/Production.ps1)
#>
[CmdletBinding()]
param()
#=================================================
...
if ($WindowsPhase -eq 'WinPE') {
    ...
    Start-OSDCloud -OSVersion 'Windows 11' -OSBuild 23H2 -OSEdition Pro -OSLicense Retail -SkipAutopilot -SkipODT -Restart
}
#endregion
#=================================================
#region Specialize
...
if ($WindowsPhase -eq 'OOBE') {
    ...
    elseif ($TestAutopilotProfile -eq $false) {
        if (((Get-CimInstance Win32_ComputerSystem).Model -eq "Virtual Machine") -and ((Get-CimInstance Win32_ComputerSystem).Manufacturer -eq "Microsoft Corporation")){
            ...
            write-host -ForegroundColor Gray '$AutopilotRegisterCommand = Get-WindowsAutopilotInfo -Online -GroupTag Enterprise -Assign'
            }
